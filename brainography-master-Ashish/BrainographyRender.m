...
if settingsStruct.saveImages
    ...
    fig = tightfig(fig);
    I = getframe;
    print('-dpng',[figstr '_coronal.png']);
    set(K,'Visible','Off');
    view([0 1 0]);
    K=camlight('left');
    pause(1.0);
    ...
    fig = tightfig(fig);
    I = getframe;
    print('-dpng',[figstr '_sagittal.png']);
    set(K,'Visible','Off');
    close(fig);
end
...
if renderProps.singleColorFlag
    ...
    for k=1:numparts
        ...
        WB(ccVol==k) = 1;
        renderCell(WB,renderProps.singleColor,opacity);
    end
else
    for i = 1:size(numberROI,1)
        if ~isnan(regionValues(i,:))
...
sznode = renderScale*abs(nodeVals);
